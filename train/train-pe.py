...
#python train-pe.py --dim_model 256 --dim_expansion 256 --dim_latent 128 --num_head 8 --num_layer 2 --dropout 0.5 --lr 0.0003 --epochs 10 --batch_size 256 --max_len 40
...
pe_list = [spe.tokenize(smi).split(' ') for smi in smi_list]
print(f'Number of data: {len(smi_list)}')

smi_dic = get_dic(pe_list)
inv_dic = {v:k for k, v in smi_dic.items()}
token_list = parallel_f(tokenize, pe_list)
